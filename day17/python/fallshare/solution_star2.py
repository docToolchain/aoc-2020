...
def read_input_file(input_file_path, input_cycles):
    ...
    with p.open() as f:
        ...
        for z in range(-cycles, cycles +1):
            full_z_map.append(copy.deepcopy(start_data))

        full_map = list()
        for w in range(-cycles, cycles +1):
            full_map.append(copy.deepcopy(full_z_map))

        #read in stat state from file        
        for y in range(0, len(raw_rows)):
            for x in range(0, len(raw_rows[y])):
                full_map[cycles][cycles][x + cycles][y + cycles ] = raw_rows[x][y]     
    return full_map
...
def process_map(start_map, cycles):
    ...
    for i in range(0, cycles):
        ...
        for w in range(1,len(current_map) - 1): 
            for z in range(1,len(current_map[0]) - 1):
                for y in range(1,len(current_map[0][0]) - 1):
                    for x in range(1,len(current_map[0][0][0]) - 1):
                        neighbours = get_neighbours(current_map,w,z,y,x)                           
                        if neighbours == 3:
                            new_map[w][z][y][x] = "#"
                        elif neighbours == 2 and current_map[w][z][y][x] == "#":
                            new_map[w][z][y][x] = "#"
                        else:
                            new_map[w][z][y][x] = "."
        current_map = copy.deepcopy(new_map)
       # print_map(current_map[cycles])
    return current_map

def get_neighbours(new_map,w,z,y,x):
    neighbours = 0
    for dw in [-1, 0 ,1]:
        for dz in [-1, 0 ,1]:
            for dy in [-1, 0 ,1]:
                for dx in [-1, 0 ,1]:
                    if not (dw == 0 and dz == 0 and dy == 0 and dx == 0):
                        if new_map[w+dw][z+dz][y+dy][x+dx] == "#":
                            neighbours += 1
    return neighbours

def get_active_coordinates(current_map):
    active_cooridnates = 0
    for w in range(0,len(current_map)):
        for z in range(0,len(current_map[0])):
            for y in range(0,len(current_map[0][0])):
                for x in range(0,len(current_map[0][0][0])):
                    if current_map[w][z][y][x] == "#":
...
if __name__ == "__main__":
    ...
    print(f"Star 2: number of active coordinates is {get_active_coordinates(final_map)}")