...
class System{
    ...
    private lengthW: number
    private lowerX: number = 0
    private lowerY: number = 0
    private lowerZ: number = 0
    private lowerW: number = 0
    constructor(points: string[][]){
        this.points = points.map((line,y) => line.map((v,x)=>new Point(x,y,0,0,v))).flat()
    ...
    public getNeighbours(point: Point, points: Point[]){
        const range = {
            ...
            w:[point.w -1,point.w,point.w +1],
        }
        return points.filter(p => range.x.includes(p.x) 
                        && range.y.includes(p.y) 
                        && range.z.includes(p.z)
                        && range.w.includes(p.w))
                     .filter(p => !(p.x === point.x && p.y === point.y && p.z === point.z && p.w === point.w))
    }
    private getPoint(x:number,y:number,z:number,w:number,points:Point[]){
        let p = points.filter(p => p.x === x && p.y === y && p.z === z && p.w === w)
        const point = p.length > 0 ? p[0] : new Point(x, y, z, w, '.');
        if(p.length === 0 ) {
        ...
        for(let x=this.lowerX; x<this.lengthX-Math.abs(this.lowerX); x++){
            for(let y=this.lowerY; y<this.lengthY-Math.abs(this.lowerY); y++){
                for(let z=this.lowerZ; z<this.lengthZ-Math.abs(this.lowerZ); z++){
                    for(let w=this.lowerW; w<this.lengthW-Math.abs(this.lowerW); w++){
                        const p = this.getPoint(x,y,z,w,this.points)
    ...
    public print(){
        ...
        for(let z=this.lowerZ; z<this.lengthZ-Math.abs(this.lowerZ); z++){
            ...
            for(let y=this.lowerY; y<this.lengthY-Math.abs(this.lowerY); y++){
                let line = ''
                for(let x=this.lowerX; x<this.lengthX-Math.abs(this.lowerX); x++){
                    const p = this.getPoint(x,y,z,0,this.points)
