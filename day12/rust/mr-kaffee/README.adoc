...
In the second part, you have to figure out how change (x, y) coordinates when the waypoint rotates about the relative origin (the ship).

I use the following data structures (maybe a bit of overhead):

[source,rust,numbered]
.`Instr` for a single instruction
----
include::src/lib.rs[tags=Instr]
----
...
The instructions are applied one by one as follows (with `use_wp` false for part 1 and true for part 2):
[source,rust,numbered]
----
